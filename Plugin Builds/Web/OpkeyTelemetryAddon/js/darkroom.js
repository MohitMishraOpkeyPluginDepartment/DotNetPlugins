let cropportion = null;
let only_rotate_image = false;
! function () {
    "use strict";
    ...
    window.Darkroom = Darkroom, Darkroom.plugins = [], Darkroom.prototype = {
        ...
        _postTransformation: function(newImage) {
            newImage && (this.sourceImage = newImage), this.refresh(function() {
                if (only_rotate_image) {
                    const historyPluginInstance = this.plugins['history'];
                    if (historyPluginInstance && typeof historyPluginInstance._onTranformationApplied === "function") {
                        // historyPluginInstance._onTranformationApplied();
                        this.dispatchEvent("core:transformation")
                    }
                    if(cropportion!=null){
                        this.dispatchEvent("core:transformation")
                        only_rotate_image = false;
                    }
                }
                else {
                    this.dispatchEvent("core:transformation")
...
function() {
    ...
    Darkroom.plugins.rotate = Darkroom.Plugin.extend({
        ...
        rotateLeft: function() {
            if (cropportion != null) {
                this.darkroom.plugins.crop.rotate(-90, false);
            } else {
                this.rotate(-90);
            }
        },
        rotateRight: function() {
            if (cropportion != null) {
                this.darkroom.plugins.crop.rotate(90, true);
            } else {
                this.rotate(90);
            }
        },
        rotate: function(angle) {
			only_rotate_image = true;
...
function() {
    ...
    Darkroom.plugins.crop = Darkroom.Plugin.extend({
        ...
        onMouseDown: function(event) {
        	if(this.IsRelativeCropping()==true){
        		return;
        	}
            this.SetFlagForRelativeCropping(false);
        	if (this.hasFocus()) {
                var canvas = this.darkroom.canvas;
                canvas.calcOffset();
        ...
        onMouseMove: function(event) {
            ...
            if (null !== this.startX && null !== this.startY) {
                ...
                if(diffX>1 && diffY>1){
                	this.SetFlagForRelativeCropping(true);
                }
                var halfX=diffX/2;
                var halfY=diffY/2;
                var cenx=this.startX+halfX;
                var ceny=this.startY+halfY;
                sessionStorage.setItem("drawLine_x",cenx);
                sessionStorage.setItem("drawLine_y",ceny);
        ...
        SetFlagForRelativeCropping:function(flag)
        {
        	sessionStorage.setItem("canDoRelativeCrop",flag.toString());
        },
        IsRelativeCropping:function()
        {
        	var status=document.body.getAttribute("relativeCroppingStatus");
        	if(status==null)
        	{
        		return false;
        	}
        ...
        rotate: function (angle, rotateRight) {
            var Angle = (this.darkroom.image.getAngle() + angle) % 360;
            var radians = Angle * Math.PI / 180;
            this.darkroom.image.rotate(Angle);
            this.darkroom.canvas.renderAll();
            var width, height;
            height = Math.abs(this.darkroom.image.getWidth() * Math.sin(radians)) + Math.abs(this.darkroom.image.getHeight() * Math.cos(radians)), width = Math.abs(this.darkroom.image.getHeight() * Math.sin(radians)) + Math.abs(this.darkroom.image.getWidth() * Math.cos(radians)), this.darkroom.canvas.setWidth(width), this.darkroom.canvas.setHeight(height), this.darkroom.canvas.centerObject(this.darkroom.image), this.darkroom.image.setCoords(), this.darkroom.canvas.renderAll()
            this.adjustCropZoneAfterRotation(Angle, rotateRight);
        },
        adjustCropZoneAfterRotation: function (Angle, rotateRight) {
            // Check if crop zone exists
            if (this.cropZone) {
                if (Angle < 0) {
                    Angle += 360;
                }
                // Original crop zone coordinates
                var originalLeft = this.cropZone.left;
                var originalTop = this.cropZone.top;
                var originalWidth = this.cropZone.width;
                var originalHeight = this.cropZone.height;
                // Image dimensions
                var imageWidth = this.darkroom.image.width;
                var imageHeight = this.darkroom.image.height;
                var newLeft, newTop, newWidth, newHeight;

                if (Angle === 90) {
                    if (rotateRight) {
                        newLeft = imageHeight - originalTop - originalHeight;
                        newTop = originalLeft;
                        newWidth = originalHeight;
                        newHeight = originalWidth;
                    } else {
                        newLeft = originalTop;
                        newTop = imageWidth - originalLeft - originalWidth;
                        newHeight = originalWidth;
                        newWidth = originalHeight;
                    }
                } else if (Angle === 180) {
                    if (rotateRight) {
                        newLeft = imageWidth - originalHeight - originalTop;
                        newTop = originalLeft;
                        newWidth = originalHeight;
                        newHeight = originalWidth;
                    }
                        ...
                        newLeft = originalTop;
                        newTop = imageHeight - originalLeft - originalWidth;
                        newWidth = originalHeight;
                        newHeight = originalWidth;
                    }
                } else if (Angle === 270) {
                    if (rotateRight) {
                        ...
                        newLeft = originalTop;
                        newTop = imageWidth - originalWidth - originalLeft;
                        newWidth = originalHeight;
                        newHeight = originalWidth;
                    }
                } else {
                    // Angle is 0 or reset to original
                    if (rotateRight) {
                        newLeft = imageWidth - originalHeight - originalTop;
                        newTop = originalLeft;
                        newWidth = originalHeight;
