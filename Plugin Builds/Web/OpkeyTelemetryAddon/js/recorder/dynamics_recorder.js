//init Oracle Fusion
var MSDynamics = function() {

}

MSDynamics.prototype.IsElementIsVisible = function(el) {
	if (el) {
...
MSDynamics.prototype.getMSDynamicsLabelComponent = function(el) {
	// ////debugger;
...
MSDynamics.prototype.checkPreviousSiblingForOF = function(el, lbl_element,
	id_Attr_Value) {
	var lbl_elem = null;
...
MSDynamics.prototype.checkNextSiblingForOF = function(el, lbl_element,
	id_Attr_Value) {
...
MSDynamics.prototype.checkParentNodeForOF = function(el, lbl_element,
	id_Attr_Value) {
	var iterationrate = 0;
...
MSDynamics.prototype.checkChildNodeForOF = function(el, lbl_element,
	id_Attr_Value) {
...
MSDynamics.prototype.checkInputElementsForOF = function(el) {
	try{
...
MSDynamics.prototype.getSeleniumAccessorsForOF = function(el) {
	var locators = [];
	try {
...
MSDynamics.prototype.GetNodeFromXath = function(_refNode, _xpath) {
	if (_refNode == null) {
		_refNode = document;
	}
	var opkey_results = [];
...
MSDynamics.prototype.FusionGetPopUp = function() {
...
MSDynamics.prototype.GetObjectHeader = function() {
...
MSDynamics.prototype.GetSpanLabelComponent = function(_component) {
...
MSDynamics.prototype.GetLabelProperties = function(_component, ref_object) {
	...
	try {
		...
		if(_component.nodeName=="INPUT"){
			var _id=_component.getAttribute("data-id");
			if(_id){
				var labelsArray=document.getElementsByTagName("LABEL");
				for(var lia=0;lia<labelsArray.length;lia++){
					var _labelfor=labelsArray[lia].getAttribute("for");
					if(_labelfor){
						if(_labelfor.indexOf(_id)>-1){
							_label=labelsArray[lia];
							break;
						}
					}
				}
		...
		if(_label==null){
			_label = this.GetSpanLabelComponent(_component);
		}
		
		if (_label == null) {
			_label = this.getMSDynamicsLabelComponent(_component);
		}
		if (_label == null) {
		...
		if (_label != null) {
			...
			if (_ariaLabel != null) {
				ref_object["MD:label:text"] = _ariaLabel.replace(", (Blank)", "").replace(", Lookup","");
				ref_object["MD:arialabel"] = _ariaLabel.replace(", (Blank)", "").replace(", Lookup","");
				ref_object["logicalname"] = _ariaLabel.replace(", (Blank)", "").replace(", Lookup","");
...
MSDynamics.prototype.GetCompositeKeywords = function(_element, ref_object) {
...
MSDynamics.prototype.GetClosestElement = function(element, selector) { // Find
	// closest
	// Required
	// Element
...
MSDynamics.prototype.GetTableProp = function(_currentelement) {
