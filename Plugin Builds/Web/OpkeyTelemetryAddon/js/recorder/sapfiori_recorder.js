//init Oracle Fusion
var SAPFIORI = function() {

}

SAPFIORI.prototype.IsElementIsVisible = function(el) {
	if (el) {
...
SAPFIORI.prototype.getOracleFusionLabelComponent = function(el) {
	// ////debugger;
...
SAPFIORI.prototype.checkPreviousSiblingForOF = function(el, lbl_element,
		id_Attr_Value) {
	var lbl_elem = null;
...
SAPFIORI.prototype.checkNextSiblingForOF = function(el, lbl_element,
		id_Attr_Value) {
...
SAPFIORI.prototype.checkParentNodeForOF = function(el, lbl_element,
		id_Attr_Value) {
	var iterationrate = 0;
...
SAPFIORI.prototype.checkChildNodeForOF = function(el, lbl_element,
		id_Attr_Value) {
...
SAPFIORI.prototype.checkInputElementsForOF = function(el) {
...
SAPFIORI.prototype.getSeleniumAccessorsForOF = function(el) {
	var locators = [];
	try {
...
SAPFIORI.prototype.GetNodeFromXath = function(_refNode, _xpath) {
	if (_refNode == null) {
		_refNode = document;
	}
	var opkey_results = [];
...
SAPFIORI.prototype.FusionGetPopUp = function() {
...
SAPFIORI.prototype.GetObjectHeader = function() {
...
SAPFIORI.prototype.GetLabelProperties = function(_component, ref_object) {
	...
	try {
		...
		if(_label==null){
			debugger;
			if(_component.nodeName=="SPAN"){
				var idOFspan=_component.id;
				if(idOFspan!=null){
				idOFspan=idOFspan.replace("-arrow-label","");
				idOFspan=idOFspan.replace("-arrow","");
				idOFspan=idOFspan.replace("-vhi","");
				idOFspan=idOFspan+"-inner";
				var inputElement=document.getElementById(idOFspan);
				var inputAreaLabeledBy=inputElement.getAttribute("aria-labelledby");
				if(inputAreaLabeledBy!=null){
					var labelElement=document.getElementById(inputAreaLabeledBy);
		...
		if (_header) {
			ref_object["SAPFIORI:Header"] = _header;
		}
		if (_label != null) {
			// debugger;
			ref_object["SAPFIORI:label:text"] = _label.textContent;
			...
			for (var _li = 0; _li < _locators.length; _li++) {
				//_locators[_li][1] = "SAPFIORI:" + _locators[_li][1];
...
SAPFIORI.prototype.GetCompositeKeywords = function(_element, ref_object) {
...
SAPFIORI.prototype.GetClosestElement = function(element, selector) { // Find
																			// closest
																			// Required
																			// Element
...
SAPFIORI.prototype.GetTableProp = function(_currentelement) {
