...
Public Module Check
    ...
    Function AndroidSdk()
        ...
        Dim androidHome = Environment.GetEnvironmentVariable("android_home", EnvironmentVariableTarget.User)

        Dim isBuildToolsFound As Boolean
        Dim isPlatformsFound As Boolean
        Dim isPlatformsToolsFound As Boolean

        If androidHome IsNot Nothing Then

            'user set the android home check the path of andriod home set sucessfully 

            Try
                For Each Dir As String In System.IO.Directory.GetDirectories(androidHome)

                    Dim dirInfo As New System.IO.DirectoryInfo(Dir)

                    If dirInfo.ToString.ToLower.Equals(androidHome.ToString.ToLower & "\build-tools") Then
                        isBuildToolsFound = True
                    End If

                    If dirInfo.ToString.ToLower.Equals(androidHome.ToString.ToLower & "\platforms") Then
                        isPlatformsFound = True
                    End If

                    If (dirInfo.ToString.ToLower.Equals(androidHome.ToString.ToLower & "\platform-tools")) Then
                        isPlatformsToolsFound = True
                    End If
                Next
            Catch ex As System.IO.DirectoryNotFoundException
                ' When path of android home not an direcory or not prperly set
                Return Nothing
            End Try

        End If

        If isBuildToolsFound And isPlatformsFound And isPlatformsToolsFound Then
            Info("Android Sdk found at " & androidHome)
            Return androidHome
        End If

        Warn("The ANDROID_HOME environment variable is not set to the Android SDK root directory path. Checking along PATH for adb.")
        Return Nothing
    End Function
