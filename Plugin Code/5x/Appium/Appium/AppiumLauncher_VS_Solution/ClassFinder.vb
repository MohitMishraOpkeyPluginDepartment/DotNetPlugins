...
Module ClassFinder
    ...
    Sub perform(ByVal args As String(), ByVal java_process As Tuple(Of String, Integer))
        ...
        If CInt(p1.Item2) = 9 Then
            javaArgs = String.Format("-jar --add-modules java.se.ee ""{0}"" ""{1}"" ""{2}"" ", OpkeyPluginBaseJarFileName, _DefaultPluginLocation_, OpkeyPluginBaseJarFileName)
            p = java_process
        Else
            javaArgs = String.Format("-jar ""{0}"" ""{1}"" ""{2}"" ", OpkeyPluginBaseJarFileName, _DefaultPluginLocation_, OpkeyPluginBaseJarFileName)
            p = java_process
        End If
