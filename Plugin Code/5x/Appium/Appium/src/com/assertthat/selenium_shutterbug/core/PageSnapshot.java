...
public class PageSnapshot extends Snapshot {
	 ...
	 * {@link #highlight(WebElement)}} and adding provided text above highlighted
	 * element.
	 *
	 * @param element WebElement to be highlighted with Color.red and line width 3
	 * @param text    test to be places above highlighted element with Color.red,
	 *                font "Serif", BOLD, size 20
	 * @return instance of type PageSnapshot
	 */
	...
	/**
	 * Highlight WebElement within the page, same as in
	 * {@link #highlight(WebElement)} but providing ability to override default
	 * color, font values.
	 *
	 * @param element      WebElement to be highlighted
	...
	public PageSnapshot highlightWithText(WebElement element, Color elementColor, int lineWidth, String text,
		...
		try {
			...
			image = ImageProcessor.addText(image, coords.getX(), coords.getY() - textFont.getSize() / 2, text,
					textColor, textFont);
		} catch (RasterFormatException rfe) {
			throw new ElementOutsideViewportException(ELEMENT_OUT_OF_VIEWPORT_EX_MESSAGE, rfe);
		}
